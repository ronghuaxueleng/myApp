...
public abstract class ModelBase {
    ...
    public void sendOkHttpPost(final ParamsBuilder paramsBuilder, final NetWorkListener netWorkListener) {
        ...
        EasyOk.post()
                .url(paramsBuilder.getUrl())
                .tag(tag)
                .paramType(paramsBuilder.getParamType())
                .mediaType(paramsBuilder.getMediaType())
                //内部已经做了null处理
                .headers(paramsBuilder.getHeads())
                //内部已经做了null处理
                .params(paramsBuilder.getParams())
                ...
                .enqueue(new ResultCall() {
                    ...
                    public void onSuccess(final String response) {
                        ...
                        if (type == null) {
                            //如果type不带，那么返回string
                            EasyOk.getInstance().getmDelivery().post(() -> netWorkListener.onNetCallBack(paramsBuilder.getCommand(), response));
                            return;
                        }
                        Object successBean = null;
                        try {
